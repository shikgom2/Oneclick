...
- 공개 주소: **`https://180.83.245.145/booth/`** (443, Let's Encrypt IP 인증서, `/booth/` 만 서비스). 인쇄 QR 은 `https://180.83.245.145/booth/start/` (찍으면 번호·확인 코드가 채워진 구글 폼이 바로 열린다).
...
sequenceDiagram
    ...
    V->>S: 인쇄 QR 로 GET /booth/start/ (키오스크는 POST /booth/kiosk/new/)
    S-->>V: 참가자 번호 SF-042 발급, 번호·확인 코드를 채운 구글 폼 주소로 바로 302 (키오스크는 폼 iframe)
    V->>F: 폼이 바로 열림, 작성 후 제출
    F-->>V: 확인 메시지 (접수 완료, 부스로 오세요, 리포트는 체험 뒤 태블릿 QR)
    F->>S: POST /booth/api/survey/ (폼 키)
    Note over V,T: 체험자가 부스로 이동
    T->>S: GET /booth/api/pending/ (가린 이름 목록, 태블릿 키)
    T->>S: POST /booth/api/measurement/ (스태프가 고른 번호)
    S-->>T: report_url
    T-->>V: 체험 뒤 태블릿 화면의 리포트 QR 을 휴대폰으로 찍음
    V->>S: GET /booth/p/{token}/ (개인 페이지), 페이지가 POST /booth/p/{token}/generate/
    S->>C: 신원 문항을 뺀 설문 응답 + 지표 설명 (측정 숫자·이름·번호·토큰 없음)
    C-->>S: 구조화된 JSON 리포트
    S-->>V: 리포트 화면 (표는 서버가 그림, PDF로 저장 = 브라우저 인쇄)
    Note over V,S: 설문 접수 뒤 같은 브라우저로 인쇄 QR 을 다시 찍으면 폼이 아니라 개인 페이지 (접수 전이면 같은 번호의 폼, 다른 앱·시크릿 창이면 새 번호)
    Note over S: 발급 14일 후 booth_purge(매시) 가 삭제
```

| 경로 | 누가 | 설명 |
|---|---|---|
| `GET /booth/start/` | 휴대폰 (인쇄된 QR) | 번호 발급 후 **번호·확인 코드를 미리 채운 구글 폼으로 바로 302** (`/survey/` 와 같은 주소, 토큰 없음). 설문이 이미 접수된 번호면 개인 페이지로(다시 제출해 응답을 덮어쓰지 않게). `BOOTH_FORM_URL` 이 없거나 http(s) 가 아니면, 또는 `BOOTH_FORM_NUMBER_ENTRY` 가 없으면 번호가 보이는 개인 페이지로. 12시간 안에 **처음 연 같은 브라우저(같은 앱)** 로 다시 찍으면 같은 번호(쿠키는 휴대폰이 아니라 브라우저 단위라 다른 스캐너 앱·시크릿 창에서는 새 번호). `?new=1` 은 새 번호 |
...
| `GET /booth/p/{token}/` | 체험자 | 개인 페이지 (설문 전 / 설문 후 / 동의 없음 / 생성 중 / 실패 / 리포트). 휴대폰 흐름에서는 체험 태블릿의 리포트 QR 을 찍거나, 설문 접수 뒤 같은 브라우저로 인쇄 QR 을 다시 찍을 때 열린다 |
| `GET /booth/p/{token}/survey/` | 체험자 | 개인 페이지의 설문하기·설문 다시 작성하기 버튼. 번호·확인 코드를 미리 채운 구글 폼으로 리다이렉트 |
...
| `routers.py`, `checks.py`, `apps.py` | booth 모델만 booth SQLite 로 보내는 라우터, 설정 체크(booth.E001/E002, 배포 경고 W001~W005), WAL 켜기 |
...
| `BOOTH_FORM_URL` | (없음) | 구글 폼 응답 주소 `https://docs.google.com/forms/d/e/<ID>/viewform`. 없거나 http(s) 주소로 해석되지 않으면(`https://[` 같은 오타 포함) 인쇄 QR 이 폼 대신 개인 페이지를 열고, 그 페이지의 설문하기와 키오스크가 '설문 주소 미설정' 안내(503) |
| `BOOTH_FORM_NUMBER_ENTRY` | (없음) | 번호 문항의 미리 채우기 파라미터 `entry.123456789` (숫자만 넣어도 됨). **반드시 설정**: 인쇄 QR 이 폼을 바로 열어 휴대폰 체험자는 번호를 따로 볼 화면이 없다. `BOOTH_FORM_URL` 만 있고 이 값이 없으면 인쇄 QR 은 폼 대신 번호가 보이는 개인 페이지를 연다(경고 booth.W005) |
...
4. 확인: 휴대폰으로 인쇄 QR(`/booth/start/`)을 찍어 바로 열린 폼의 번호 칸에 `SF-0xx`, 확인 코드 칸에 8자 코드가 들어 있어야 한다
   (키오스크 폼도 같다). 이 확인도 번호를 하나 발급한다.
...
| 프레젠테이션 → **확인 메시지** | 아래 [제출 확인 메시지](#제출-확인-메시지-권장) | 휴대폰 체험자는 제출 뒤 이 메시지만 보고 부스로 온다. 기본 문구(`응답이 기록되었습니다.`)로는 다음에 할 일을 알 수 없다 |
...
인쇄 QR 은 폼을 바로 열기 때문에, 휴대폰 체험자가 제출 뒤에 보는 화면은 **구글 폼의 확인 메시지뿐**이다(개인 페이지를 거치지 않는다).
폼 편집 화면 **설정 → 프레젠테이션 → 확인 메시지** 의 '수정' 에 아래 문구를 넣는다. 같은 폼을 키오스크 iframe 에서도 쓰므로 두 경우에 모두 맞게 썼다.

```text
설문이 접수되었습니다. 참여해 주셔서 감사합니다.
이제 체험 부스로 와 주세요. 스태프가 확인한 뒤 측정과 체험을 시작합니다.
체험이 끝나면 체험 태블릿 화면에 리포트 QR 코드가 나옵니다. 휴대폰 카메라로 찍으면 AI 리포트를 볼 수 있습니다.
(개인정보 동의 문항에 동의하지 않으신 분도 측정 체험은 하실 수 있지만, 리포트는 제공되지 않습니다.)
```

- 이 화면은 자동으로 바뀌지 않는다. 휴대폰 화면이 리포트로 바뀌는 것은 **체험 태블릿의 리포트 QR 을 찍어** 개인 페이지를 열었을 때다(확실한 방법).
  처음 QR 을 연 같은 브라우저로 인쇄 QR 을 다시 찍어도 열리지만, 다른 앱으로 찍으면 새 번호의 폼이 열린다([8](#휴대폰으로-참여)).
- 확인 메시지는 고정 문구라 번호·이름을 넣을 수 없다. 스태프는 체험 태블릿 대기 목록의 가린 이름(`홍○동`)으로 번호를 고른다.
...
1. 인쇄된 QR(`https://180.83.245.145/booth/start/`)을 찍으면 번호(예: `SF-042`)와 확인 코드가 채워진 **구글 폼이 바로 열린다**(누를 버튼 없음).
   번호·확인 코드는 고치지 않고 작성해 제출한다.
2. 제출하면 폼의 [제출 확인 메시지](#제출-확인-메시지-권장)가 "접수되었으니 부스로 오세요, 리포트는 체험 뒤 태블릿 QR 로" 를 안내한다.
3. 부스에서 스태프가 체험 태블릿 대기 목록에서 가린 이름(`홍○동`)으로 번호를 고르고 측정·체험을 한다.
4. 체험이 끝나면 **체험 태블릿 화면의 리포트 QR** 을 휴대폰으로 찍는다 → 개인 페이지가 열리고 리포트 생성(1~2분) 뒤 리포트가 뜬다. **PDF로 저장** 은 브라우저 인쇄다.

- 휴대폰에 남은 폼 확인 화면은 **리포트로 자동으로 바뀌지 않는다**(개인 페이지를 열어 둔 채 기다리는 흐름이 아니다).
  리포트는 **체험 태블릿의 리포트 QR** 로 여는 것이 확실하다. 처음 QR 을 연 같은 브라우저로 인쇄 QR 을 다시 찍어도 열린다.
- 다시 찍었을 때 같은 번호로 알아보는 것은 휴대폰이 아니라 **브라우저 쿠키**다. 처음과 다른 앱(예: 기본 카메라로 열었다가 네이버·카카오톡 QR 스캐너로 찍음.
  앱마다 내장 브라우저가 쿠키를 따로 둔다)이나 시크릿 창·쿠키 삭제 뒤에 찍으면 **새 번호의 폼**이 열린다.
  이 폼은 제출하지 않는다(제출하면 같은 사람이 번호 두 개로 대기 목록에 뜬다). 체험 뒤 태블릿의 리포트 QR 을 쓴다.
- **설문이 접수된 뒤 같은 브라우저로 인쇄 QR 을 다시 찍으면** 폼이 아니라 개인 페이지(설문 접수·동의 없음·생성 중·리포트)가 열린다.
  같은 번호로 폼을 다시 제출해 앞의 응답과 리포트를 덮어쓰지 않게 하기 위해서다. 동의를 고치려면 그 페이지의 **설문 다시 작성하기** 를 쓴다.
- 설문이 서버에 접수되기 전(작성 중 창을 닫음, 제출 후 보통 수 초 안)에 같은 브라우저로 다시 찍으면 같은 번호의 폼이 다시 열린다. 발급 후 12시간이 지나면 새 번호를 받는다.
- 가족이 같은 휴대폰으로 하려면 앞사람이 **제출한 뒤** 같은 브라우저로 인쇄 QR 을 다시 찍어 열린 개인 페이지 아래 **새 번호 받기**(`/booth/start/?new=1`)를 누른다 → 새 번호의 폼이 바로 열린다.
  제출 전에 찍으면 앞사람 번호의 폼이 열리고, 같은 번호로 나중에 낸 응답이 앞의 응답을 덮어쓴다.
...
- [ ] 휴대폰으로 인쇄 QR 을 찍으면 번호·확인 코드가 채워진 폼이 바로 열림(키오스크 폼도 동일), 제출 뒤 [제출 확인 메시지](#제출-확인-메시지-권장)가 보임,
      같은 카메라 앱(같은 브라우저)으로 QR 을 다시 찍으면 폼이 아니라 개인 페이지('설문이 접수되었습니다')가 열림, 체험 태블릿의 리포트 QR 로도 그 개인 페이지가 열림
...
- 리포트 생성은 **관람객이 개인 페이지를 열 때** 시작된다. 태블릿이 상태를 조회한다고 생성되지는 않는다.
  휴대폰 흐름에서는 인쇄 QR 이 폼을 바로 열어 관람객이 개인 페이지를 열어 두지 않으므로, **측정 뒤 `report_url` QR 을 반드시 띄워** 찍게 한다.
...
| 화면 | 조건 | 내용 |
|---|---|---|
| 설문 전 | 설문 없음 | 번호를 크게, **설문하기** (새 탭). 휴대폰 흐름에서는 보통 보이지 않는다(인쇄 QR 이 폼을 바로 연다). `BOOTH_FORM_URL`·`BOOTH_FORM_NUMBER_ENTRY` 가 없을 때나 설문 전인 개인 페이지 주소를 직접 열 때 보인다 |
| 동의 없음 | 동의 문항 중 하나라도 거부 | 리포트를 제공할 수 없고 이름·응답·측정값을 저장하지 않았다는 안내, 설문 다시 작성하기(측정도 다시) |
| 설문 후 | 설문 있음, 측정 없음 | `<이름>`님, 체험 태블릿으로 이동 안내. 설문 접수 뒤 같은 브라우저로 인쇄 QR 을 다시 찍으면 이 화면(또는 동의 없음·생성 중·리포트)이 열린다 |
...
- 개인 페이지는 **열려 있는 동안에만** 4초마다 상태를 확인해 화면을 바꾼다. 휴대폰 흐름에서 제출 직후 휴대폰에 남는 것은 구글 폼 확인 화면이라,
  체험 태블릿의 리포트 QR(또는 같은 브라우저로 인쇄 QR 재스캔)으로 개인 페이지를 열어야 리포트가 보인다.
...
- 모든 booth 응답에 `Cache-Control: no-store`, `Referrer-Policy: no-referrer`, `X-Robots-Tag: noindex`. https 가 아니면 403, 오류는 한국어 404/500 화면(DEBUG 화면 없음).
  인쇄 QR(`/start/`)·설문하기(`/survey/`)가 구글 폼으로 보내는 302 에도 붙어, 개인 페이지의 '새 번호 받기'에서 출발해도 토큰 URL 이 Referer 로 구글에 가지 않는다.
  구글 폼 주소에는 번호·확인 코드만 있고 토큰은 없다.
...
| 설문을 냈는데 인쇄 QR 을 다시 찍으면 폼이 또 열림 / 개인 페이지가 '설문 전' 그대로 | 다시 열린 폼의 **번호가 전과 다르면** 처음과 다른 브라우저·앱(다른 QR 스캐너 앱, 시크릿 창, 쿠키 삭제)으로 찍어 새 번호가 발급된 것이다. 제출하지 말고 체험 뒤 태블릿의 리포트 QR 을 쓴다. **번호가 같으면** 설문이 서버에 접수되지 않았다. Apps Script **실행** 목록의 `onBoothFormSubmit` 실패 사유 → `checkConnection`. 번호 칸이 비었거나 고쳐 씀(404/400), 확인 코드를 고침(400) |
| Apps Script 로그 `확인 코드가 맞지 않습니다` (400) | 번호나 코드를 고쳐 씀. 접수되지 않았으므로 처음 연 같은 브라우저로 인쇄 QR 을 다시 찍으면 같은 번호의 폼이 다시 열린다(키오스크는 새 체험자 시작) |
...
| 인쇄 QR 이 폼 대신 번호만 있는 개인 페이지를 열고, 설문하기가 '설문 주소 미설정' | `BOOTH_FORM_URL` 없음 또는 http(s) 주소가 아님(로그 `BOOTH_FORM_URL 이 http(s) 주소가 아니라 무시합니다`). 고친 뒤 uWSGI 재시작, 체험자는 그 페이지의 설문하기나 QR 재스캔 |
| 폼에 번호·코드가 안 채워짐 / 설문하기는 되는데 인쇄 QR 이 폼 대신 번호가 보이는 개인 페이지를 엶 | `BOOTH_FORM_NUMBER_ENTRY`(없으면 경고 booth.W005, 로그 `BOOTH_FORM_NUMBER_ENTRY 가 비어 있어`), `BOOTH_FORM_CODE_ENTRY` 확인. 번호 문항이 없으면 폼만 보고는 번호를 알 수 없어 인쇄 QR 이 개인 페이지로 보내고, 체험자는 설문하기에서 화면의 번호를 직접 적어야 한다(번호 없는 제출은 400). 고친 뒤 uWSGI 재시작, 같은 브라우저로 QR 을 다시 찍으면 채워진 폼이 열린다 |
| 제출했는데 휴대폰 화면이 리포트로 바뀌지 않음 | 정상이다. 제출 뒤 휴대폰에는 구글 폼 확인 메시지만 남는다. 체험 뒤 태블릿의 리포트 QR 을 찍는다(처음 연 같은 브라우저라면 인쇄 QR 재스캔으로도 열린다) |
...
| `manage.py check` 에 booth.W001~W005 | 3. 환경변수 표의 해당 항목 |
